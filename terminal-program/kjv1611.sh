#!/bin/sh
# kjv1611: Read the Word of God from your terminal
...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_data kjv1611.tsv | awk -v cmd=list "$(get_data kjv1611.awk)"
		exit
	elif [ "$1" = "-W" ]; then
		export KJV1611_NOLINEWRAP=1
		shift
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data kjv1611.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data kjv1611.awk)" | ${PAGER}
	done
	exit 0
fi

get_data kjv1611.tsv | awk -v cmd=ref -v ref="$*" "$(get_data kjv1611.awk)" | ${PAGER}
